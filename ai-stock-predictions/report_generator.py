...
def create_full_report(
    ...
    def fig_to_html(fig):
        ...
        return to_html(fig, include_plotlyjs=False, full_html=False) if fig else ""
    ...
    forecast_chart_html = fig_to_html(forecast_chart_fig)
    ...
    bb_chart_html = fig_to_html(bb_fig)
    rsi_fig, rsi_conclusion = plot_rsi(hist_data_for_ta.copy(), ticker)
    rsi_chart_html = fig_to_html(rsi_fig)
    macd_lines_fig, macd_conclusion = plot_macd_lines(hist_data_for_ta.copy(), ticker)
    macd_lines_chart_html = fig_to_html(macd_lines_fig)
    macd_hist_fig, _ = plot_macd_histogram(hist_data_for_ta.copy(), ticker)
    macd_hist_chart_html = fig_to_html(macd_hist_fig)
    ...
    <style>
      ...
      .report-container { max-width: 1200px; margin: 2rem auto; padding: 1.5rem 2rem; background: #ffffff; box-shadow: 0 8px 25px rgba(0,0,0,0.1); border-radius: 12px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #333; line-height: 1.6; }
      ...
      .historical-chart .plotly-graph-div { padding-top: 1rem; }
      ...
      .plotly-graph-div { margin: auto; width: 100%; }
    ...
    html_sections = [ # (Same sections as before)
        ...
        ("Moving Average Convergence Divergence (MACD)",
         ...
         f'<div class="indicator-conclusion">{macd_conclusion or ""}</div>',
    ...
    report_body_content = "" # (Same assembly logic as before)
    for item in html_sections:
        ...
        if html_content_or_chart:
             ...
             if section_class == "tech-chart-macd-combined": report_body_content += f"  {html_content_or_chart}\n"
             ...
             else: report_body_content += f"  {html_content_or_chart}\n"
             report_body_content += f"""</div>\n"""

    # ****** ADDED PLOTLY SCRIPT BACK TO <HEAD> ******
...
<body>
    <div class="report-container">
        <h1 class="report-title">{ticker} Stock Forecast & Analysis</h1>
        {report_body_content}
    </div>
</body>
</html>"""
    ...
    return report_path, full_html # Return value includes HTML, though app.py might ignore it now